...
def compute(pattern):
    ...
    if len(pattern) % 4 == 0:
        salienceprofile = [5, 1, 2, 1, 3, 1, 2, 1, 4, 1, 2, 1, 3, 1, 2, 1, 4, 1, 2, 1, 3, 1, 2, 1, 4, 1, 2, 1, 3, 1, 2,
                           1]  # using binary LHL profile
    if len(pattern) % 3 == 0:
        salienceprofile = [5, 1, 2, 1, 2, 1, 3, 1, 2, 1, 2, 1, 4, 1, 2, 1, 2, 1, 3, 1, 2, 1, 2,
                           1]  # using ternary LHL profile
...
def lassoMatch(target, data, alpha):
    ...
    for axis in range(len(coordinates[0])):
        ...
        clf = linear_model.Lasso(alpha=alpha, copy_X=True, fit_intercept=True,
                                 max_iter=1000, normalize=True, positive=False, precompute=False, tol=0.0001,
                                 warm_start=False)
...
def lassoMatchSelect(target, data, desclista, alpha):
    ...
    output = []
    filteredmtx = []
    finaldescriptorlist = list(
        descriptorList16)  # start with a complete list of descriptors copied from this original list
...
def patts2space(targetCoords, patts, desclista):
    ...
    distances = np.zeros(
        [len(filteredmtx), len(filteredmtx)])  # create an empty matrix with cols and rows = number of patts
    ...
    for p in tnoname:
        for step, s in enumerate(p):
            ...
            for m in s:
                if m != 0:
                    s2eight.append(
                        GM2eight(m))  # Remap all MIDI notes to only 8 instruments (-1 if instrument not mapable)
                else:
            ...
            allsteps2l.append(
                combo2l8[str(lowpack)] + combo2l8[str(hipack)])  # here we have a list of steps with max 8 instruments
...
def rs_gen(listofpatterns):
    ...
    rsDesc = ['lowsync', 'midD', 'hisyness', 'hiD', 'hiness', 'stepD',
              'noi']  # the set of descriptors to be extracted form the patterns
